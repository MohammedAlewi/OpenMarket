...
open class ProductRepository(private val productDao: ProductDao, private val productCommentDao: ProductCommentDao,
                        private val userProductDao: UserProductDao,private val commentDao: CommentDao
){
    lateinit var activity: MainActivity
    open fun insertProduct(product: Product,user_id:Long){
    ...
    open fun getProductId(id:Long):LiveData<Product>{
        if (activity.isConnected()){
            GlobalScope.launch(Dispatchers.IO){
    ...
    open fun getProductsByUsername(username:String):LiveData<List<Product>>{
        if (activity.isConnected()){
            GlobalScope.launch(Dispatchers.IO){
    ...
    open fun getAllProducts():LiveData<List<Product>>{
        if (activity.isConnected()){
            GlobalScope.launch(Dispatchers.IO){
    ...
    open fun updateProduct(product: Product){
        if (activity.isConnected()){
            GlobalScope.launch(Dispatchers.IO) {
    ...
    open fun deleteProduct(product: Product){
        if (activity.isConnected()){
            GlobalScope.launch(Dispatchers.IO) {
