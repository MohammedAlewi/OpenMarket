...
class ProductDetailFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        if(!product.userName.startsWith('@')){product.userName="@"+product.userName}
        fragmentProductDetailBinding.product=product
        fragmentProductDetailBinding
            .comLst=CommentListener(this,product,view,commentViewModel,activity as MainActivity,
        ...
        productViewModel.getCommentForProduct(product.id).observe(this,androidx.lifecycle.Observer {
        ...
        ratingViewModel.getRatingByProductId(product.id).observe(this,androidx.lifecycle.Observer {
            ratings -> ratings.let { if(ratings.filter{ rating -> rating.username==username }.isEmpty()){
                ratingViewModel.saveRating(Rating(username?:"unknown",0.0,product.id))}
            }
        })


        ratingViewModel.getViewersNumber(product.id).observe(this,androidx.lifecycle.Observer {
            num -> num.let {view.view_no.text=num.toString()}
        })


        ratingViewModel.getRatingValue(product.id).observe(this,androidx.lifecycle.Observer {
            rating_no->rating_no.let {if(!rating_no.isNaN()){view.product_rating.text=rating_no.toString()}else{view.product_rating.text="0.0"} }
        })
...
class CommentListener(private var context: ProductDetailFragment,private var product: Product,
    ...
    fun buttonClicked() {
        ...
        var date=Date().day.toString() +"/"+Date().month+"/"+(Date().year.toInt()+1900)+" "+(Date().hours%12)+":"+Date().minutes
    ...
    fun navigateToUser(){
        ...
        if (product.userName.contains('@')){username=product.userName.substring(1)}
        userViewModel.getUserByUsername(username).observe(context,androidx.lifecycle.Observer {
                userObj-> userObj.let {
                    var arg=Bundle()
    ...
    fun addRating(){
        var username=activity.getSharedPreferences("user_login",Context.MODE_PRIVATE).getString("username","unknown")
        ratingViewmodel.getRatingByUsername(username ?:"unknown").observe(context,androidx.lifecycle.Observer {
            ratings -> ratings.forEach { rating -> rating.let {
